...
public class Command_Kick extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (message.length > 0 && message[0].equalsIgnoreCase(Bot.BotPrefix + "kick")) {
        ...
        if (message.length == 1 && message[0].equalsIgnoreCase(Bot.BotPrefix + "kick")) {
            if (isAllowed) {
                EmbedBuilder EmbedRules = new EmbedBuilder();
                EmbedRules.setTitle("Incorrect Format");
                EmbedRules.setColor(Color);
                EmbedRules.addField("Format:", "" + Bot.BotPrefix + "kick [@user] [reason]", false);
        ...
        if (message.length == 2 && message[0].equalsIgnoreCase(Bot.BotPrefix + "kick")) {
            if (isAllowed) {
                EmbedBuilder EmbedRules = new EmbedBuilder();
                EmbedRules.setTitle("Incorrect Format");
                EmbedRules.setColor(Color);
        ...
        if (message.length > 2 && message[0].equalsIgnoreCase(Bot.BotPrefix + "kick")) {
            if (isAllowed) {
                ...
                e.getGuild().kick(MentionedMember).queue();
                ...
                EmbedBuilder EmbedRules = new EmbedBuilder();
                EmbedRules.setTitle("Successfully Kicked");
                EmbedRules.setColor(Color);
                EmbedRules.addField("Success", e.getMember().getUser().getAsTag() + ", you have kicked " + MentionedUser.getAsTag(), false);
                ...
                EmbedBuilder logs = new EmbedBuilder()
                        .setTitle("Kick Logs")
                        .setColor(Color)
                        .addField("User", e.getMember().getUser().getAsTag() + ", has kicked " + MentionedUser.getAsTag(), false)
