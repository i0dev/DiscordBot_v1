...
public class Extra_UnBlacklist extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (message.length > 0 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unblacklist")) {
        ...
        if (message.length == 1 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unblacklist")) {
            if (isAllowed) {
                EmbedBuilder EmbedRules = new EmbedBuilder();
                EmbedRules.setTitle("Incorrect Format");
                EmbedRules.setColor(Color);
                EmbedRules.addField("Format:", "" + Bot.BotPrefix + "unblacklist [@user]", false);
        ...
        if (message.length == 2 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unblacklist")) {
            if (isAllowed) {
                ...
                for (String s : BlacklistedGet) {
                    if (MentionedMember.getId().equals(s)) {
                        isBlacklisted = true;
                    }
                }
                ...
                EmbedBuilder EmbedRules = new EmbedBuilder();
                if (!isBlacklisted) {
                    EmbedRules.setTitle("Error while Un-Blacklisting");
                    EmbedRules.setColor(Color);
                    EmbedRules.addField("Error", e.getMember().getUser().getAsTag() + ", " + MentionedUser.getAsTag() + " is **Not** blacklisted!", false);
                    EmbedRules.setTimestamp(Bot.now);
                    EmbedRules.setFooter(Bot.WaterMark, Bot.Logo);
                    channel.sendMessage(EmbedRules.build()).queue(message1 -> {
                        e.getMessage().delete().queue();
                    ...
                    for (int i = 0; i < BlacklistedGet.size(); i++) {
                        if (BlacklistedGet.get(i).equals(MentionedMember.getId())) {
                            BlacklistedGet.remove(i);
                        }
                    }
                    ...
                    EmbedRules.setColor(Color);
                    EmbedRules.addField("Success", e.getMember().getUser().getAsTag() + ", you have Un-Blacklisted " + MentionedUser.getAsTag() + " From using all bot commands", false);
                    EmbedRules.setTimestamp(Bot.now);
                    EmbedRules.setFooter(Bot.WaterMark, Bot.Logo);
                    channel.sendMessage(EmbedRules.build()).queue(message1 -> {
                        e.getMessage().delete().queue();
                    ...
                    EmbedBuilder logs = new EmbedBuilder()
                            .setTitle("Un-Blacklist Logs")
                            .setColor(Color)
                            .addField("User", e.getMember().getUser().getAsTag() + ", has un-blacklisted " + MentionedUser.getAsTag() + " From using all bot commands", false)
