...
public class TEST_GiveawayReRoll extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (message.length > 0 && message[0].equalsIgnoreCase(Bot.BotPrefix + "greroll")) {
        ...
        if (message.length == 1 && message[0].equalsIgnoreCase(Bot.BotPrefix + "greroll")) {
            if (isAllowed) {
                EmbedBuilder EmbedRules = new EmbedBuilder();
                EmbedRules.setTitle("Incorrect Format");
                EmbedRules.setColor(Color);
                EmbedRules.addField("Format:", "" + Bot.BotPrefix + "greroll [Message ID]", false);
        ...
        if ((message.length == 2) && message[0].equalsIgnoreCase(Bot.BotPrefix + "greroll")) {
            if (isAllowed) {
                ...
                if (!GiveawayExists) {
                    ...
                    if (!GiveawayEnded) {
                        EmbedBuilder EmbedFirst = new EmbedBuilder()
                                .setTitle(BotName + " Giveaways")
                                .setColor(java.awt.Color.RED)
                                .addField("Error", "**" + e.getMember().getUser().getAsTag() + "**, That giveaway is still running! You can only re-roll finished giveaways!", false)
                                .setTimestamp(now)
                                .setFooter(Bot.WaterMark, BotLogo);
                        channel.sendMessage(EmbedFirst.build()).queue(message1 -> {
                            e.getMessage().delete().queue();
                        ...
                        EmbedBuilder EmbedFirst = new EmbedBuilder()
                                .setTitle(BotName + " Giveaways")
                                .setColor(Color)
                                .addField("Success", "**" + e.getMember().getUser().getAsTag() + "**, You re-rolled that giveaway, The winner will be posted in the channel the giveaway is in!", false)
                                .setTimestamp(now)
                                .setFooter(Bot.WaterMark, BotLogo);
                        channel.sendMessage(EmbedFirst.build()).queue(message1 -> {
                            e.getMessage().delete().queue();
                        ...
                        ArrayList<String> CurrentWinnersID = (ArrayList<String>) CurrentGiveaway.get("WinnersID");
                        ArrayList<String> TotalReactedMemberIDS = (ArrayList<String>) CurrentGiveaway.get("TotalReactedMemberIDS");
                        Color CurrentColor = java.awt.Color.decode(CurrentColorHexCode);
                        TextChannel GiveawayChannel = Bot.jda.getGuildById(CurrentGuildID).getTextChannelById(CurrentChannelID);


                        String WinnersString = "";
                        ArrayList<User> Winners = new ArrayList<>();
                        if (TotalReactedMemberIDS.size() == 0) {
                            WinnersString = "`No Winners :cry:`";
                        } else {
                            Random random = new Random();
                            for (int j = 0; j < Integer.parseInt(CurrentWinnersAmount); j++) {
                                int randomint = random.nextInt(TotalReactedMemberIDS.size());
                                try {
                                    Winners.add(Bot.jda.getGuildById(CurrentGuildID).getMemberById(TotalReactedMemberIDS.get(randomint)).getUser());
                        ...
                        EmbedBuilder EmbedReroll = new EmbedBuilder()
                                .setTitle("Giveaway Re-Roll!")
                                .addField(":trophy: New Winners", WinnersString, false)
                                .setThumbnail(CurrentBotLogo)
                                .setTimestamp(LocalTime)
                                .setColor(CurrentColor)
                                .setFooter("Re-Rolled at ", BotLogo);

                        GiveawayChannel.sendMessage(EmbedReroll.build()).queue();
                        ...
                        ArrayOfGiveaways.add(CurrentGiveaway);
