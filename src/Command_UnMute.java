...
public class Command_UnMute extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent e) {
        ...
        if (message.length > 0 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unmute")) {
        ...
        if (message.length == 1 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unmute")) {
            if (isAllowed) {
                ...
                EmbedRules.addField("Format:", "" + Bot.BotPrefix + "unmute [@user]", false);
        ...
        if (message.length == 2 && message[0].equalsIgnoreCase(Bot.BotPrefix + "unmute")) {
            if (isAllowed) {
              ...
              if (e.getGuild().getMember(MentionedUser).getRoles().contains(mutedRole)) {
                  e.getGuild().removeRoleFromMember(MentionedMember, mutedRole).queue();


                  EmbedBuilder EmbedRules = new EmbedBuilder();
                  EmbedRules.setTitle("Successfully Un-Muted");
                  EmbedRules.setColor(Color);
                  EmbedRules.addField("Success", e.getMember().getUser().getAsTag() + ", you have un-muted " + MentionedUser.getAsTag(), false);
                  EmbedRules.setTimestamp(Bot.now);
                  EmbedRules.setFooter(Bot.WaterMark, Bot.Logo);
                  channel.sendMessage(EmbedRules.build()).queue(message1 -> {
                      e.getMessage().delete().queue();
                  ...
                  EmbedBuilder logs = new EmbedBuilder()
                          .setTitle("Un-Mute Logs")
                          .setColor(Color)
                          .addField("User", e.getMember().getUser().getAsTag() + ", has un-muted " + MentionedUser.getAsTag(), false)
                  ...
                  EmbedRules.setColor(Color);
                  EmbedRules.addField("Error", e.getMember().getUser().getAsTag() + ", " + MentionedUser.getAsTag() + " is not muted, so they cannot be unmuted", false);
                  EmbedRules.setTimestamp(Bot.now);
                  EmbedRules.setFooter(Bot.WaterMark, Bot.Logo);
                  channel.sendMessage(EmbedRules.build()).queue(message1 -> {
                      e.getMessage().delete().queue();
